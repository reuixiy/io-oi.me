...
<video src="QmSNG5zhinQDiZPnstQmr1dbiYz34uoCuxjQP1HJTQe2Nn" title="深圳的天空"></video>
...
接下来修改 config.txt，在文件末的 `dtparam=audio=on` 下方添加 `start_x=1` 以开启摄像头[^1]，也可添加 `disable_camera_led = 1` 以将摄像头上的 LED 灯关闭。
...
[^1]: 也可进入系统后，通过 `raspi-config` 配置。
[^2]: 这也可能会影响后面的系统更新，系统会自动选择就近的源，下载速度更快。
